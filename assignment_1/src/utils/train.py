...
def train(model, epochs=10, lr=0.001, criterion=None, delete_loaders=True, save_dir='trained'):
    ...
    model_path_acc = f'{save_dir}/{model.get_name()}_{timestamp}_acc_{best_acc:.2f}.pth'
    model_path_loss = f'{save_dir}/{model.get_name()}_{timestamp}_loss_{best_loss:.4f}.pth'
    ...
    print(f"\nBest val loss: {best_loss:.4f} (epoch {best_loss_epoch})")
    print("=" * 60)
    ...
    plt.tight_layout()
    cm_path = f'{save_dir}/{model.get_name()}_{timestamp}_confusion_matrix.png'
    plt.savefig(cm_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"\nConfusion matrix saved to: {cm_path}")
    print(f"Model saved to: {model_path_acc}")
    print("=" * 60)
